...
support using Flask-Babel. It allows forcing a locale via a URL
parameter and falls back to the best match from the `Accept-Language`
header if the parameter is not present.
"""
...
def get_locale() -> str:
    ...
    Checks for a `locale` parameter in the request arguments. If present and
    supported, uses it as the locale. Otherwise, falls back to the
    `Accept-Language` header to determine the best language
    match among the supported locales.
...
def index() -> str:
    ...
    can be determined from the `locale` URL
